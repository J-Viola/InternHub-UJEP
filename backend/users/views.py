...
def login(request):
    ...
    login_url = f"{settings.STAG_WS_URL}/login?originalURL={callback_url}?basicAuth=1"
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({'error': f'Login request failed with status {response.status_code}'}, status=500)
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Retrieved additional user details for ticket {ticket}")
            else:
                logger.warning(f"Failed to get user details from STAG WS: {response.status_code}")

        except Exception as e:
            logger.error(f"Error requesting STAG user details: {str(e)}")
        response = redirect(settings.FRONTEND_URL + "?auth_success=true")
        response.set_cookie('session_id', request.session.session_key, httponly=True, secure=True)
        # Redirect to frontend with success
        return response

    except Exception as e:
        logger.error(f"Auth callback error: {str(e)}")
        return JsonResponse({'error': f"Failed to process authentication data {str(e)}"}, status=400)
...
def aresJustice(request):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({'error': 'Failed to fetch data from ARES'}, status=response.status_code)
