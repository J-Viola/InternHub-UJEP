...
class ChangePasswordViewTests(APITestCase):
    def setUp(self):
        ...
        self.user = OrganizationUser.objects.create_user(
            email="test@org.com", password="OldPassword123", first_name="Test", last_name="Org", is_active=True
        )
        self.url = reverse("users:change-password")

    def test_change_password_success(self):
        self.client.force_authenticate(user=self.user)
        data = {"old_password": "OldPassword123", "new_password": "NewPassword456", "new_password_confirm": "NewPassword456"}
    ...
    def test_change_password_incorrect_old(self):
        self.client.force_authenticate(user=self.user)
        data = {"old_password": "WrongPassword", "new_password": "NewPassword456", "new_password_confirm": "NewPassword456"}
        response = self.client.post(self.url, data)
    ...
    def test_change_password_mismatch(self):
        self.client.force_authenticate(user=self.user)
        data = {"old_password": "OldPassword123", "new_password": "NewPassword456", "new_password_confirm": "MismatchPassword"}
        response = self.client.post(self.url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class PasswordResetFlowTests(APITestCase):
    def setUp(self):
        self.user = OrganizationUser.objects.create_user(
            email="reset@org.com", password="OldPassword123", first_name="Reset", last_name="User", is_active=True
        )
    ...
    def test_reset_confirm_success(self):
        ...
        data = {"uidb64": uid, "token": token, "new_password": "NewStrongPassword123", "new_password_confirm": "NewStrongPassword123"}
