    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        has_access = (
            StudentPractice.objects.filter(
                Q(contract_document_id=doc.document_id)
                | Q(content_document_id=doc.document_id)
                | Q(feedback_document_id=doc.document_id)
            )
            .filter(
                Q(user=request.user)
                | Q(practice__contact_user=request.user)
                | Q(practice__employer__organization_users=request.user)
            )
            .exists()
        )
