...
class EmployerUserRoleSerializer(serializers.ModelSerializer):
    """
    Serializer pro model EmployerUserRole
    - id: primární klíč (read-only)
    - employer: nested EmployerProfile (read-only)
    - employer_id: PK zaměstnavatele (write-only)
    - user: PK uživatele (write-only)
    ...
    def validate(self, data):
        ...
        user = data.get('user')
        if EmployerUserRole.objects.filter(employer=employer, user=user).exists():
...
class StudentPracticeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['student_practice_id', 'title', 'logo', 'application_date', 'approval_status', 'progress_status', 'cancelled_by_user_info']
