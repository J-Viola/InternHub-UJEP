    ...
    def validate(self, data):
        ...
        if hours < 0:
            raise serializers.ValidationError(
                "Počet dokončených hodin nesmí být záporný."
            )
        return data
...
class StudentPracticeCardSerializer(serializers.ModelSerializer):
    ...
    def get_student_practice_documents(self, obj):
        ...
        student_practice_documents.append(
            {"id": obj.contract_document.document_id, "type": "contract"}
        )
        student_practice_documents.append(
            {"id": obj.content_document.document_id, "type": "content"}
        )
        student_practice_documents.append(
            {"id": obj.feedback_document.document_id, "type": "feedback"}
        )
